...
template <class T> TAllocate TTVector<T>::vector_allocator(sizeof(TTVector<T>), 100);
...
template <class T> TIter<T> TTVector<T>::begin(){
    return TIter<T>(array);
}

template <class T> TIter<T> TTVector<T>::end(){
    return TIter<T>(array + size);
}
