#include "TAllocationBlock.h"
#include <iostream>

// Смотри комментарии в .h файле !!!

TAllocationBlock::TAllocationBlock(size_t size, size_t count) : 
_size(size),_count(count) {
    _used_blocks = (char*)malloc(_size*_count);
    _free_blocks = (void**)malloc(sizeof(void*) * _count);
    
    for(size_t i=0;i<_count;i++) _free_blocks[i] = _used_blocks+i*_size;
    _free_count = _count;
    std::cout << "TAllocationBlock: Память инициализирована" << std::endl;
}

void *TAllocationBlock::allocate() {
    void *result = nullptr;

    if(_free_count > 0) { 
        result = _free_blocks[_free_count-1];
        _free_count--;
        std::cout << "TAllocationBlock: Веделено " << (_count-_free_count) <<
        " of " << _count << std::endl;
    } else {
        std::cout << "TAllocationBlock: Нет памяти" << std::endl;
    }
    return result;
}

void TAllocationBlock::deallocate(void *pointer) {
    std::cout << "TAllocationBlock: Блок был возвращён"<< std::endl;
    _free_blocks[_free_count] = pointer;
    _free_count ++;
}

bool TAllocationBlock::has_free_blocks() {
    return _free_count>0;
}

TAllocationBlock::~TAllocationBlock() {
    if(_free_count<_count) {
        std::cout << "TAllocationBlock: Конец" << std::endl;
    } else {
        std::cout << "TAllocationBlock: Память освобождена" << std::endl;
    }
    delete _free_blocks;
    delete _used_blocks;
}